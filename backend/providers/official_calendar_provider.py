...
class OfficialCalendarProvider:
    """Official-source fallback for USD/GBP macro release schedules."""
    ...
    BLS_HIGH = (
        ...
        "Retail Sales", "Productivity and Costs", "Job Openings and Labor Turnover Survey",
        "U.S. Import and Export Price Indexes", "Average Hourly Earnings",
    )
    ONS_HIGH = (
        "consumer price inflation", "cpi", "labour market", "earnings and employment",
        "retail sales", "gdp", "producer price inflation",
        "services producer price inflation", "gross domestic product",
    )
    ...
    def get_events(self, days_before: int = 1, days_after: int = 30) -> list[dict]:
        ...
        unique = {(e["title"], e["currency"], e["time"]): e for e in events}
    ...
    def _get_bls_events(self, start: datetime, end: datetime) -> list[dict]:
        ...
        while cursor <= end_month:
            try:
                ...
                events.extend(self._parse_bls_text(self._html_text(response.text), start, end))
            except Exception:
                pass
            cursor = cursor.replace(year=cursor.year + 1, month=1) if cursor.month == 12 else cursor.replace(month=cursor.month + 1)
        return events

    def _parse_bls_text(self, text: str, start: datetime, end: datetime) -> list[dict]:
        # BLS list view is Date / Time / Release; HTML parsing can separate cells,
        # so this accepts arbitrary whitespace between all fields.
        pattern = re.compile(
            r"(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),\s+"
            r"([A-Za-z]+)\s+(\d{1,2}),\s+(\d{4})\s+"
            r"(?:(\d{1,2}:\d{2})\s+(AM|PM)\s+)?"
            r"(.+?)(?=\s+(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),\s+"
            r"[A-Za-z]+\s+\d{1,2},\s+\d{4}\b|\s+NOTE:\s+All times|$)",
        ...
        for match in pattern.finditer(text):
            ...
            try:
                if clock and ampm:
                    local = datetime.strptime(f"{month_name} {day} {event_year} {clock} {ampm}", "%B %d %Y %I:%M %p").replace(tzinfo=eastern)
                else:
                    local = datetime.strptime(f"{month_name} {day} {event_year}", "%B %d %Y").replace(hour=9, minute=0, tzinfo=eastern)
            except ValueError:
                continue
    ...
    def _get_fed_events(self, start: datetime, end: datetime) -> list[dict]:
        ...
        pattern = re.compile(r"\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2})-(\d{1,2})\b", re.IGNORECASE)
        for match in pattern.finditer(text):
            month, _first_day, second_day = match.groups()
            try:
                year_match = re.search(r"202[0-9]", text[max(0, match.start() - 120):match.start() + 120])
                if not year_match:
                    continue
                local = datetime(int(year_match.group()), month_names[month.lower()], int(second_day), 14, 0, tzinfo=eastern)
            except (ValueError, AttributeError, KeyError):
                continue
            event_time = local.astimezone(timezone.utc)
            if start <= event_time <= end:
                events.append(self._event("Federal Reserve FOMC interest rate decision", "USD", event_time, "High", "Federal Reserve"))
        return events

    def _get_ons_events(self, start: datetime, end: datetime) -> list[dict]:
        try:
            response = self.session.get(self.ONS_URL, params={
                "limit": 100, "sort": "release_date_asc",
                "fromDate": start.date().isoformat(), "toDate": end.date().isoformat(),
        ...
        if not isinstance(payload, dict):
            return []
        items = payload.get("releases") or payload.get("results") or payload.get("items") or []
        ...
        for item in items:
            ...
            if isinstance(description, dict):
                title = str(item.get("title") or description.get("title") or "").strip()
                raw_date = item.get("release_date") or description.get("releaseDate")
            else:
                title = str(item.get("title") or description or "").strip()
                raw_date = item.get("release_date") or item.get("releaseDate") or item.get("date")
            normalized = title.lower()
            if not title or not any(term in normalized for term in self.ONS_HIGH) or not raw_date:
                continue
            try:
                value = str(raw_date)
                if "T" in value:
                    ...
                    parsed = datetime.fromisoformat(value).replace(hour=7, minute=0, tzinfo=london)
    ...
    def _event(title: str, currency: str, event_time: datetime, impact: str, source: str) -> dict:
        ...
        return {
            ...
            "title": title, "currency": currency, "time": event_time.isoformat(),
            "impact": impact, "category": "macro", "actual": None,
            "forecast": None, "previous": None,
            "country": "United States" if currency == "USD" else "United Kingdom",
            "source": source, "minutes": round((event_time - now).total_seconds() / 60),
        }
