...
export default function PALAssistant({ context, title = "PAL Intelligence", subtitle = "Ask about the live evidence", accent = "cyan", quickPrompts = DEFAULT_PROMPTS }: Props) {
  ...
  return <>
    {open ? <div className="fixed bottom-5 right-5 z-[260] w-[min(420px,calc(100vw-24px))] overflow-hidden rounded-3xl border border-white/10 bg-[#07100f]/95 shadow-[0_30px_100px_rgba(0,0,0,.65)] backdrop-blur-xl">
      <div className={`border-b border-white/[.07] ${tone.header} px-4 py-3`}><div className="flex items-center justify-between gap-3"><div><div className={`text-[9px] font-bold uppercase tracking-[.22em] ${tone.label}`}>{title}</div><div className="mt-1 text-[10px] text-zinc-500">{subtitle}</div></div><button type="button" onClick={() => setOpen(false)} aria-label="Close PAL chat" className="flex h-8 items-center gap-1.5 rounded-xl border border-white/[.07] px-2.5 text-[9px] font-semibold text-zinc-500 transition hover:border-white/15 hover:text-white"><span className="text-sm leading-none">×</span> Close</button></div></div>
