...
const DEFAULT_PROMPTS = ["What is today's brief?", "What news matters today?", "Explain the current bias?"];
const ACCENT = {
...
export default function PALAssistant({ context, title = "PAL Intelligence", subtitle = "Ask about the live evidence", accent = "cyan", quickPrompts = DEFAULT_PROMPTS }: Props) {
  ...
  const page = String((context as Record<string, unknown>).page ?? "dashboard");

  // A Journal conversation must never leak into Dashboard, and vice versa.
  useEffect(() => {
    setMessages([]);
    setInput("");
    setError("");
  ...
  return <>
    {open ? <div className="fixed bottom-5 right-5 z-[260] w-[min(420px,calc(100vw-24px))] overflow-hidden rounded-3xl border border-white/10 bg-[#07100f]/95 shadow-[0_30px_100px_rgba(0,0,0,.65)] backdrop-blur-xl">
      <div className={`border-b border-white/[.07] ${tone.header} px-4 py-3`}><div><div className={`text-[9px] font-bold uppercase tracking-[.22em] ${tone.label}`}>{title}</div><div className="mt-1 text-[10px] text-zinc-500">{subtitle}</div></div></div>
      <div className="max-h-[430px] min-h-[210px] space-y-3 overflow-y-auto p-4">
        {!messages.length ? <div className="rounded-2xl border border-white/[.06] bg-white/[.02] p-4"><div className="text-sm font-semibold text-white">Evidence-first assistant</div><p className="mt-2 text-[11px] leading-5 text-zinc-500">I use only the evidence available on this page. Ask a specific question and PAL will answer that question rather than repeating the previous response.</p><div className="mt-4 flex flex-wrap gap-2">{quickPrompts.slice(0,2).map(prompt=><button key={prompt} type="button" onClick={()=>void ask(prompt)} className={`rounded-full border px-3 py-1.5 text-[9px] ${tone.chip}`}>{prompt}</button>)}</div></div> : null}
      ...
      <div className="border-t border-white/[.07] p-3">
        <form onSubmit={event=>{event.preventDefault();void ask(input)}}><div className="flex items-center gap-2 rounded-2xl border border-white/[.08] bg-black/30 p-1.5"><input value={input} onChange={event=>setInput(event.target.value)} placeholder="Ask PAL anything about the evidence…" className="min-w-0 flex-1 bg-transparent px-2 text-[11px] text-white outline-none placeholder:text-zinc-700"/><button type="submit" disabled={!input.trim()||loading} className={`rounded-xl ${tone.button} px-3 py-2 text-[10px] font-bold text-[#031014] disabled:opacity-30`}>Ask</button></div></form>
        <button type="button" onClick={() => setOpen(false)} className="mt-2 w-full rounded-xl border border-white/[.07] bg-white/[.02] px-3 py-2 text-[10px] font-semibold text-zinc-500 hover:border-white/15 hover:text-white">Close PAL Chat</button>
      </div>
    </div> : null}
