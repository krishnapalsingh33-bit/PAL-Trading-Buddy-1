      ...
      <div className="relative flex flex-col gap-5 xl:flex-row xl:items-stretch">
        <div className="min-w-[270px] flex-1 rounded-2xl border border-white/[0.07] bg-black/20 p-5">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[9px] font-semibold uppercase tracking-[0.22em] text-cyan-300">Actionable Macro Layer</div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full rounded-full bg-current opacity-70 transition-all duration-700" style={{ width: `${Math.min(100, Number(today.confidence ?? 0))}%` }} /></div>
            <span className="text-[10px] text-zinc-500">{Number(today.confidence ?? 0)}% confidence</span>
          </div>
          <p className="mt-4 text-[10px] uppercase tracking-wider text-zinc-600">Scope: today only · recalculated from current evidence</p>
        ...
        <div className="grid flex-[1.35] gap-3 sm:grid-cols-3">
