...
type Props = { markets: Record<string, Quote>; report?: any };
...
export default function MarketWorkspace({ markets, report }: Props) {
  ...
  const status = String(quote.status ?? "UNAVAILABLE").toUpperCase();
  return (
    <section className="mt-5 overflow-hidden rounded-3xl border border-white/[.07] bg-white/[.018] shadow-[0_25px_90px_-65px_rgba(34,211,238,.35)]">
      <div className="flex flex-col gap-4 border-b border-white/[.06] p-5 lg:flex-row lg:items-end lg:justify-between">
        <div><div className="text-[9px] font-bold uppercase tracking-[.22em] text-cyan-300">Live Market Workspace</div><h2 className="mt-2 text-xl font-semibold text-white">One-click market monitor</h2><p className="mt-1 text-xs text-zinc-600">Select an asset to expand live price, movement, and available market context.</p></div>
        <div className="rounded-xl border border-white/[.07] bg-black/20 px-3 py-2 text-[8px] uppercase tracking-wider text-zinc-600">Provider-backed · no invented prices</div>
      </div>
        ...
        <div className="border-b border-white/[.06] lg:border-b-0 lg:border-r">
          <div className="p-3">
            {ASSETS.map((asset) => {
              ...
              return <button key={asset.key} type="button" onClick={() => setSelected(asset.key)} className={`mb-2 w-full rounded-2xl border p-3 text-left transition-all duration-300 ${active ? "border-cyan-300/25 bg-cyan-300/[.05] shadow-[0_0_28px_-18px_rgba(34,211,238,.9)]" : "border-white/[.06] bg-black/15 hover:-translate-y-0.5 hover:border-white/[.12]"}`}><div className="flex items-center justify-between"><div><div className="text-[10px] font-semibold text-white">{asset.label}</div><div className="mt-1 text-[8px] uppercase tracking-wider text-zinc-700">{asset.unit}</div></div><span className="text-[8px] uppercase tracking-wider text-zinc-600">{String(item.status ?? "—")}</span></div><div className="mt-3 flex items-end justify-between"><div><div className="text-lg font-semibold text-white">{fmtPrice(item.price)}</div><div className={`mt-1 text-[9px] font-mono ${movementClass(item.change_percent)}`}>{movement(item.change_percent)} {fmtPct(item.change_percent)}</div></div><div className={`text-2xl ${movementClass(item.change_percent)}`}>{movement(item.change_percent)}</div></div></button>;
            })}
          </div>
        </div>

        <div className="min-w-0 p-5 lg:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><div className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full ${status === "CURRENT" ? "animate-pulse bg-emerald-300" : "bg-zinc-600"}`} /><span className="text-[9px] uppercase tracking-[.2em] text-zinc-600">{status === "CURRENT" ? "LIVE" : status}</span></div><h3 className="mt-2 text-2xl font-semibold text-white">{selectedAsset.label}</h3><p className="mt-1 text-[10px] text-zinc-600">{selectedAsset.unit} · {quote.source ?? "No provider"}</p></div><div className="text-left md:text-right"><div className="text-3xl font-semibold text-white">{fmtPrice(quote.price)}</div><div className={`mt-1 text-[10px] font-mono ${movementClass(quote.change_percent)}`}>{movement(quote.change_percent)} {fmtPct(quote.change_percent)}</div></div></div>

          <div className="mt-5 rounded-2xl border border-white/[.06] bg-black/20 p-4"><div className="flex items-center justify-between"><span className="text-[8px] uppercase tracking-[.18em] text-zinc-700">Recent movement</span><span className="text-[8px] text-zinc-700">Right Now</span></div><div className="mt-4 h-44">{path ? <svg viewBox="0 0 100 40" className="h-full w-full" preserveAspectRatio="none"><polyline points={path} fill="none" stroke={Number(quote.change_percent) >= 0 ? "#34d399" : "#f87171"} strokeWidth="1.8" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" /></svg> : <div className="flex h-full items-center justify-center text-[9px] uppercase tracking-[.18em] text-zinc-700">Live movement data is warming up</div>}</div></div>

          <div className="mt-4 grid gap-4 xl:grid-cols-2">
            <div className="rounded-2xl border border-white/[.06] bg-black/20 p-4"><div className="text-[8px] uppercase tracking-[.18em] text-zinc-700">Market snapshot</div><div className="mt-3 grid grid-cols-2 gap-2 text-[9px]"><div className="rounded-xl border border-white/[.05] p-3"><div className="text-zinc-700">Today change</div><div className={`mt-1 font-mono text-sm ${movementClass(quote.change_percent)}`}>{fmtPct(quote.change_percent)}</div></div><div className="rounded-xl border border-white/[.05] p-3"><div className="text-zinc-700">Right now</div><div className="mt-1 font-mono text-sm text-zinc-300">{fmtPct(quote.right_now_momentum_percent, 3)}</div></div><div className="rounded-xl border border-white/[.05] p-3"><div className="text-zinc-700">Source</div><div className="mt-1 text-sm text-zinc-300">{quote.source ?? "—"}</div></div><div className="rounded-xl border border-white/[.05] p-3"><div className="text-zinc-700">Last update</div><div className="mt-1 text-[10px] text-zinc-500">{quote.timestamp ? new Date(quote.timestamp).toLocaleTimeString([], { hour12: false }) : "—"}</div></div></div></div>
            <div className="rounded-2xl border border-white/[.06] bg-black/20 p-4"><div className="text-[8px] uppercase tracking-[.18em] text-zinc-700">Relevant headlines</div><div className="mt-3 space-y-2">{news.length ? news.map((item: any, index: number) => <div key={`${item?.title ?? "item"}-${index}`} className="rounded-xl border border-white/[.05] bg-white/[.018] p-3"><div className="text-[10px] leading-4 text-zinc-300">{item?.title ?? "Headline"}</div><div className="mt-1 text-[8px] uppercase tracking-wider text-zinc-700">{item?.source ?? item?.provider ?? "News"}</div></div>) : <div className="rounded-xl border border-white/[.05] bg-white/[.018] p-3 text-[9px] leading-4 text-zinc-700">No asset-specific headlines are currently available from the PAL news feed.</div>}</div></div>
          </div>
        </div>
      </div>
    </section>
