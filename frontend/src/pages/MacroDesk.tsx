...
type MarketCard = { key: string; symbol: string; label: string; subtitle: string; tone: string };
...
const BACKEND_SYMBOL: Record<string, string> = Object.fromEntries(MARKETS.map((market) => [market.key, market.symbol]));

function getMarket(markets: Record<string, unknown>, key: string): MarketQuote | undefined {
    const symbol = BACKEND_SYMBOL[key];
    for (const candidate of [symbol, symbol?.toLowerCase(), key]) {
...
function analysisFor(card: MarketCard, market: MarketQuote | undefined, bias: string) {
    ...
    if (card.key === "gbpusd") return "GBP strength is being weighed against the broader USD backdrop. Use the macro view to confirm whether the current move has enough fundamental support.";
    if (card.key === "xauusd") return "Gold is being read through rates, USD pressure and risk tone. A cleaner directional read requires those drivers to agree.";
    if (change != null && Math.abs(change) > 0.7) return `${card.label} is moving with elevated momentum. ${bias === "Neutral" ? "Direction remains mixed until macro and price pressure align." : `Current macro framing is ${bias.toLowerCase()}.`}`;
    return `${card.label} is showing a quieter tape. PAL is watching cross-asset pressure, macro catalysts and fresh price movement before increasing conviction.`;
}

function quickOverview(card: MarketCard, market: MarketQuote | undefined, bias: string) {
    const change = market?.change_percent;
    return [
        market?.price != null ? `Price ${market.price.toLocaleString(undefined, { maximumFractionDigits: 4 })}` : "Live price is not currently supplied by the provider",
        change != null ? `Session move ${change >= 0 ? "+" : ""}${change.toFixed(2)}%` : "Session change is unavailable",
        `Macro bias: ${bias}`,
        market?.reason || `${card.label} is being evaluated against cross-asset pressure and current macro catalysts.`,
    ];
}
                ...
                <main className="mx-auto max-w-[1420px] px-4 py-6 sm:px-6 lg:px-8">
                    <header className="mb-5">
                        ...
                        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                            <div><h1 className="text-3xl font-semibold tracking-tight sm:text-[36px]">AI Market Bias Dashboard</h1><p className="mt-1 text-sm text-zinc-500">Where deep institutional analysis meets AI-driven clarity</p></div>
                            <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.18em] text-zinc-600"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_10px_rgba(110,231,183,.9)]" />Live macro intelligence</div>
                        </div>
                    </header>
                    ...
                    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                        {cards.map((card) => {
                            ...
                            const overview = quickOverview(card, market, bias);
                            return (
                                <article key={card.key} className="group overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0a1512]/95 shadow-[0_18px_50px_rgba(0,0,0,.28)] transition duration-200 hover:-translate-y-0.5 hover:border-emerald-300/20 hover:shadow-[0_22px_65px_rgba(0,0,0,.36)]">
                                    <div className="p-4 sm:p-5">
                                        <div className="flex items-start justify-between gap-3">
                                            <div><div className="flex items-center gap-2"><h2 className="text-[17px] font-semibold tracking-tight">{card.label}</h2><span className={`text-[10px] font-semibold ${change != null && change >= 0 ? "text-emerald-400" : "text-red-400"}`}>{displayChange(market)}</span></div><p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-zinc-600">{card.subtitle} · {card.tone}</p></div>
                                        ...
                                        <div className="mt-4 flex items-center justify-between"><div><div className="text-[10px] text-zinc-600">Confidence</div><div className="mt-1 h-1.5 w-44 overflow-hidden rounded-full bg-zinc-900 sm:w-56"><div className="h-full rounded-full bg-[#9ec8ba]" style={{ width: `${Math.max(8, Math.min(100, confidence || 45))}%` }} /></div></div><span className="text-[10px] text-zinc-500">{Math.max(0, Math.min(100, confidence || 45))}%</span></div>
                                        <div className="mt-4 rounded-xl border border-white/[0.06] bg-[#08110f] p-3.5"><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-cyan-300/80">◆ AI Analysis</div><p className="mt-2 text-[12px] leading-[1.55] text-zinc-400 line-clamp-3">{analysisFor(card, market, bias)}</p></div>
                                        <div className="mt-3 flex gap-2">
                                            <button type="button" aria-expanded={expandedCard} onClick={() => setExpanded(expandedCard ? null : card.key)} className={`flex-1 rounded-lg border px-3 py-2 text-[10px] font-semibold transition ${expandedCard ? "border-cyan-300/20 bg-cyan-300/[0.06] text-cyan-200" : "border-white/[0.08] bg-white/[0.025] text-zinc-400 hover:bg-white/[0.05] hover:text-zinc-200"}`}>{expandedCard ? "Close Overview" : "Quick Overview"} <span className="ml-1 transition-transform">{expandedCard ? "⌃" : "⌄"}</span></button>
                                        ...
                                        {expandedCard && <div className="mt-3 rounded-xl border border-cyan-300/10 bg-cyan-300/[0.025] p-3.5"><div className="text-[9px] font-semibold uppercase tracking-[0.18em] text-cyan-300/60">Quick Overview</div><ul className="mt-2 space-y-2">{overview.map((item, index) => <li key={index} className="flex gap-2 text-[11px] leading-5 text-zinc-400"><span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-cyan-300/70" />{item}</li>)}</ul></div>}
                                        <div className="mt-4 flex items-center justify-between text-[9px] uppercase tracking-wider text-zinc-700"><span>{freshness(market)}</span><span>{market?.status ?? "UNAVAILABLE"}</span></div>
                                    </div>
                                </article>
                            );
                        })}
                    ...
                    <section className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
                        <div className="rounded-2xl border border-white/[0.08] bg-[#091310] p-5 shadow-[0_18px_50px_rgba(0,0,0,.24)]"><div className="flex items-center justify-between"><div><p className="text-[10px] uppercase tracking-[0.2em] text-emerald-300/70">News intelligence</p><h2 className="mt-1 text-lg font-semibold">Recent macro catalysts</h2></div><span className="text-[10px] text-zinc-700">{headlines.length} shown</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{headlines.map((item, index) => <div key={index} className="rounded-xl border border-white/[0.05] bg-black/20 p-4"><p className="text-sm leading-5 text-zinc-300">{String((item as any).title ?? (item as any).headline ?? "Macro headline")}</p><p className="mt-2 text-[9px] uppercase tracking-widest text-zinc-700">{String((item as any).source ?? "Provider")}</p></div>)}{!headlines.length && <p className="text-sm text-zinc-600">No macro headlines currently supplied.</p>}</div></div>
                        <div className="rounded-2xl border border-white/[0.08] bg-[#091310] p-5 shadow-[0_18px_50px_rgba(0,0,0,.24)]"><div className="text-[10px] uppercase tracking-[0.2em] text-cyan-300/70">Data health</div><div className="mt-4 space-y-2">{Object.entries(sourceStatus).map(([source, status]) => <div key={source} className="flex items-center justify-between rounded-lg border border-white/[0.05] bg-black/20 px-3 py-2"><span className="text-xs text-zinc-500">{source}</span><span className="text-[9px] uppercase tracking-wider text-zinc-400">{String(status)}</span></div>)}{!Object.keys(sourceStatus).length && <p className="text-sm text-zinc-600">No macro observation sources reported.</p>}</div></div>
                    </section>
                </main>
            </div>
        </PalPageShell>
