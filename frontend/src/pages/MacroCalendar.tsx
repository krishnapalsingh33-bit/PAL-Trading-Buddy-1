...
export default function MacroCalendar({ onPageChange }: Props) {
    ...
    const events = useMemo(() => {
        const raw = [
            ...(data?.report?.news?.upcoming_events ?? []),
            ...(data?.report?.news?.recent_events ?? []),
        ] as EventItem[];
        const seen = new Set<string>();
        return raw.filter((event) => {
            const key = `${text(event, ["title", "name", "event"])}|${text(event, ["time", "scheduled_time", "datetime"])}|${text(event, ["currency", "ccy"])}|${text(event, ["impact"] )}`;
            ...
            <div className="mx-auto max-w-7xl p-5 sm:p-8">
                <header>
                    ...
                    <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-semibold">What Matters Next</h1>
                            <p className="mt-2 text-sm text-zinc-500">Market-moving events and macro context from the existing PAL feed.</p>
                        </div>
                ...
                <section className="mt-4 rounded-2xl border border-zinc-800 bg-zinc-950 p-5">
                    ...
                    <div className="space-y-3">
                        {events.map((event, index) => {
                            ...
                            return (
                                <article key={`${title}-${index}`} className="rounded-xl border border-zinc-900 bg-zinc-900/35 p-4">
                                    <div className="flex flex-wrap items-start justify-between gap-3">
                                        <div>
                                            <p className="text-[11px] uppercase tracking-widest text-zinc-600">{time}</p>
                                            <h3 className="mt-1 text-base font-semibold">{title}</h3>
                                        </div>
                                        <div className="flex gap-2">
                                            <span className="rounded-full border border-zinc-800 px-2.5 py-1 text-[10px] font-semibold text-zinc-400">{eventCurrency}</span>
                                            <span className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold ${impactClass(eventImpact)}`}>{eventImpact}</span>
                                        </div>
                                    </div>
                                    <div className="mt-4 grid gap-2 sm:grid-cols-3">
                                        <div className="rounded-lg bg-zinc-950/70 p-3"><p className="text-[10px] uppercase tracking-widest text-zinc-600">Actual</p><p className="mt-1 text-sm text-zinc-300">{actual}</p></div>
                                        <div className="rounded-lg bg-zinc-950/70 p-3"><p className="text-[10px] uppercase tracking-widest text-zinc-600">Forecast</p><p className="mt-1 text-sm text-zinc-300">{forecast}</p></div>
                                        <div className="rounded-lg bg-zinc-950/70 p-3"><p className="text-[10px] uppercase tracking-widest text-zinc-600">Previous</p><p className="mt-1 text-sm text-zinc-300">{previous}</p></div>
                                    </div>
                                </article>
                            );
                        })}
