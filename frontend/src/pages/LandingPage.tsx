    ...
    <div className="relative min-h-screen overflow-hidden bg-[#020807] text-zinc-100 selection:bg-emerald-300/20">
      ...
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold tracking-wide">PAL</div>
            <div className="text-[9px] uppercase tracking-[.22em] text-zinc-600">Trading Buddy</div>
          </div>
        </div>

        <div className="hidden items-center gap-7 text-[9px] uppercase tracking-[.18em] text-zinc-600 md:flex">
          <a href="#why" className="transition hover:text-zinc-200">Why PAL</a>
          <a href="#features" className="transition hover:text-zinc-200">Features</a>
          <a href="#method" className="transition hover:text-zinc-200">Method</a>
        </div>
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-5 pb-20 lg:px-8">
        <section className="grid gap-12 pb-20 pt-10 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:pt-20">
          <div>
            ...
            <h1 className="mt-7 max-w-4xl text-5xl font-semibold leading-[1.02] tracking-tight text-white sm:text-6xl lg:text-[72px]">Your trading desk,<br /><span className="bg-gradient-to-r from-emerald-200 via-cyan-200 to-white bg-clip-text text-transparent">with an AI beside you.</span></h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <button type="button" onClick={onEnter} className="rounded-2xl bg-emerald-300 px-5 py-3 text-xs font-bold text-[#03110c] shadow-[0_15px_40px_rgba(52,211,153,.16)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_46px_rgba(52,211,153,.22)]">Enter PAL Workspace</button>
              <span className="rounded-2xl border border-white/[.07] bg-white/[.02] px-5 py-3 text-xs text-zinc-500">Private · evidence-first · uncertainty stays visible</span>
            </div>
            <div className="mt-8 grid max-w-2xl grid-cols-2 gap-3 sm:grid-cols-4">
              <MiniStat label="Today layer" value="LIVE" tone="text-emerald-300" />
              <MiniStat label="Macro layer" value="SEPARATE" tone="text-cyan-300" />
              <MiniStat label="Journal" value="MT5" />
              <MiniStat label="AI" value="PAL" tone="text-amber-300" />
            </div>
          </div>
          ...
          <div className="relative mx-auto w-full max-w-[560px]">
            ...
            <div className="relative aspect-square overflow-hidden rounded-[2.8rem] border border-emerald-300/15 bg-[#071311]/95 shadow-[0_40px_120px_rgba(0,0,0,.55)]">
              ...
              <div className="absolute inset-x-6 bottom-6 rounded-2xl border border-white/[.06] bg-black/40 p-4 backdrop-blur-xl">
                <div className="flex items-center justify-between"><span className="text-[9px] uppercase tracking-[.2em] text-zinc-600">Current evidence</span><span className="rounded-full bg-emerald-300/10 px-2 py-1 text-[8px] font-bold text-emerald-300">READING</span></div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <div className="rounded-xl border border-white/[.05] bg-white/[.02] p-2.5"><div className="text-[8px] text-zinc-600">DXY</div><div className="mt-1 text-[10px] font-semibold text-amber-300">NEUTRAL</div></div>
                  <div className="rounded-xl border border-white/[.05] bg-white/[.02] p-2.5"><div className="text-[8px] text-zinc-600">GBP</div><div className="mt-1 text-[10px] font-semibold text-emerald-300">BULLISH</div></div>
                  <div className="rounded-xl border border-white/[.05] bg-white/[.02] p-2.5"><div className="text-[8px] text-zinc-600">GBP/USD</div><div className="mt-1 text-[10px] font-semibold text-emerald-300">BULLISH</div></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="why" className="scroll-mt-10 border-t border-white/[.06] py-16">
          <div className="max-w-2xl"><div className="text-[9px] uppercase tracking-[.24em] text-cyan-300">Why PAL exists</div><h2 className="mt-3 text-3xl font-semibold text-white">Less noise. More evidence. Better decisions.</h2><p className="mt-3 text-sm leading-6 text-zinc-500">Trading gets harder when charts, news, calendars and journals live in different places. PAL turns those layers into one workspace while keeping missing evidence visible rather than filling gaps with confident guesses.</p></div>
          <div id="features" className="mt-8 grid scroll-mt-10 gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Feature eyebrow="Today" title="Live bias" body="Keep current trading-day evidence separate from the longer-term regime so the active session remains the first decision layer." icon="⌁" />
            ...
            <Feature eyebrow="AI" title="PAL Intelligence" body="Ask natural-language questions about the evidence on your dashboard, macro desk or trading journal." icon="✦" />
